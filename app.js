...
function handleFetchCommand(entryType, ctx) {
  ...
  axios.get(dataObj.urls[entryType]).then(
    (res) => {
    ...
    (err) => {
      ...
      ctx.replyWithMarkdown(
        `Some error occured while fetching the ${entryType}s. Please report this to @ankushgarg1998`
      );
    }
  );
}
...
function handleSubscribeCommand(entryType, ctx) {
  log(
    `Subscribe ${entryType} entries hit. -> ${ctx.chat.id} : ${
  ...
  subscriptionInstance
    ...
    .then((savedSubscription) => {
      ...
      ctx.replyWithMarkdown(
        `You're now subscribed to ${entryType}s. You'll be notified as soon as any new ${entryType} is uploaded on the website. ✅`
      );
    })
    .catch((err) => {
      log(err.message);
      if (err.code === 11000) {
        ...
        log(err);
        ctx.replyWithMarkdown(
          `There was an error in subscribing. Please report this to @ankushgarg1998`
        );
      }
    });
}
...
function handleUnsubscribeCommand(entryType, ctx) {
  log(
    `Unsubscribe ${entryType} entries hit. -> ${ctx.chat.id} : ${
      ctx.chat.username || ctx.chat.first_name + " " + ctx.chat.last_name
    }`
  );
  ...
  Subscription.deleteOne(unsubscribeObject)
    .then((res) => {
    ...
    .catch((err) => {
      ...
      log(err);
      ctx.replyWithMarkdown(
        `There was an error in unsubscribing. Please report this to @ankushgarg1998`
      );
    });
}
...
function handleCronExecution(entryType) {
  ...
  axios.get(dataObj.urls[entryType]).then(
        ...
        Subscription.find({
          ...
          .then((subs) => {
            ...
            subs.forEach((sub) => {
              ...
              tg.sendMessage(
                sub.chat_id,
                `🔴 *NEW ${entryType.toUpperCase()} ALERT*\n\n` + message,
                markD
              )
          ...
          .catch((err) => {
            ...
            log(err);
            tg.sendMessage(
              dataObj.masterChatID,
              `Yo Ankush. Some error occured in ${entryType}-checker cron while fetching from db.\n`,
              markD
            )
    ...
    (err) => {
      ...
      tg.sendMessage(
        dataObj.masterChatID,
        `Yo Ankush. Some error occured in ${entryType}-checker cron.\n`,
        markD
      )
        .then((val) => log(`=> Notified Ankush.`))
        .catch((err) => {
